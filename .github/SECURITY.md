...
| TOTP secrets | Encrypted at rest with AES-256-GCM (random IV per field, authenticated). Replay-guarded per member (monotonic counter). Per-member, per-source, and global codeless-login rate limits (5 / 15 min per member; codeless: 5 / 15 min per source, 40 / 15 min global, and reaching the global ceiling asks for a member name rather than refusing every sign-in). Legacy plaintext values are auto-migrated on first boot under an active KEK. |
