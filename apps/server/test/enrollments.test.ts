...
import { ENROLLMENT_TTL_MS, EnrollmentStore, formatUserCode, normalizeUserCode } from 'csuite-core';
...
describe('normalizeUserCode', () => {
  ...
  it('rejects ambiguous letters (I, L, O, U)', async () => {
...
describe('formatUserCode', async () => {
  it('inserts a hyphen at the midpoint', async () => {
    expect(formatUserCode('KQ4M7P2H')).toBe('KQ4M-7P2H');
  });
});

describe('EnrollmentStore.mint', async () => {
  it('returns a deviceCode + userCode pair with TTL', async () => {
    const store = makeStore();
    const result = await store.mint({ sourceIp: '10.0.0.1', sourceUa: 'csuite-cli/0.0.0' });
...
describe('EnrollmentStore lifecycle', async () => {
  it('lookupByUserCode → approve → pollByDeviceCode → consume', async () => {
    const store = makeStore();
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
    ...
    const poll = await store.pollByDeviceCode(minted.deviceCode);
    ...
    const replay = await store.pollByDeviceCode(minted.deviceCode);
    expect(replay.kind).toBe('expired_token');
  });

  it('pending poll returns authorization_pending', async () => {
    const store = makeStore();
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
    const poll = await store.pollByDeviceCode(minted.deviceCode);
  ...
  it('returns slow_down on rapid back-to-back pending polls', async () => {
    let clock = 1_700_000_000_000;
    const store = makeStore(() => clock);
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
    expect((await store.pollByDeviceCode(minted.deviceCode)).kind).toBe('authorization_pending');
    clock += 100; // very fast — well under the 2.5s threshold
    expect((await store.pollByDeviceCode(minted.deviceCode)).kind).toBe('slow_down');
  });

  it('expired tokens 410 with expired_token', async () => {
    let clock = 1_700_000_000_000;
    const store = makeStore(() => clock);
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
  ...
  it('reject → poll returns access_denied + reason, then consumes', async () => {
    const store = makeStore();
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
    ...
    expect((await store.pollByDeviceCode(minted.deviceCode)).kind).toBe('expired_token');
  });

  it('lookupByUserCode reports approved/rejected/expired states', async () => {
    let clock = 1_700_000_000_000;
    const store = makeStore(() => clock);
    const m1 = await store.mint({ sourceIp: null, sourceUa: null });
    store.approve({
    ...
    const m2 = await store.mint({ sourceIp: null, sourceUa: null });
    ...
    const m3 = await store.mint({ sourceIp: null, sourceUa: null });
    clock += ENROLLMENT_TTL_MS + 1;
  ...
  it('listPending excludes approved, rejected, and expired rows', async () => {
    let clock = 1_700_000_000_000;
    const store = makeStore(() => clock);
    const a = await store.mint({ sourceIp: null, sourceUa: null });
    const b = await store.mint({ sourceIp: null, sourceUa: null });
    const c = await store.mint({ sourceIp: null, sourceUa: null });
    store.approve({
      userCode: a.userCode,
      approvedBy: 'd',
      boundMember: 'm',
    ...
    await store2.mint({ sourceIp: null, sourceUa: null });
    await store2.mint({ sourceIp: null, sourceUa: null });
  ...
  it('purgeExpired sweeps stale rows', async () => {
    let clock = 1_700_000_000_000;
    const store = makeStore(() => clock);
    await store.mint({ sourceIp: null, sourceUa: null });
    await store.mint({ sourceIp: null, sourceUa: null });
    clock += ENROLLMENT_TTL_MS + 1;
...
describe('EnrollmentStore + KEK', async () => {
  it('round-trips an issued plaintext through KEK encryption', async () => {
    const db = openDatabase(':memory:');
    const kek = Buffer.alloc(32, 0x42);
    const store = new EnrollmentStore(db, { cipher: kekFieldCipher(kek) });
    const minted = await store.mint({ sourceIp: null, sourceUa: null });
    store.approve({
      userCode: minted.userCode,
      approvedBy: 'director',
