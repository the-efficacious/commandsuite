...
- New `variables` store, `/variables/*` API and `csuite variables`
  command. Values are readable by a `secrets.manage` holder and are
  **never** passed to `registerSecretValues`.
