...
export function PageHeader({ eyebrow, title, subtitle, actions }: PageHeaderProps) {
  return (
    <div
      ...
      style="margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--ef-border)"
    >
      <div class="min-w-0">
        {eyebrow && <div class="eyebrow">{eyebrow}</div>}
        <h2
          class="font-display"
          style="font-size:30px;font-weight:700;letter-spacing:-0.02em;color:var(--ef-text);line-height:1.1;margin-top:6px"
        >
          {title}
        </h2>
        {subtitle && (
          <div style="font-family:var(--ef-font-body);font-size:13px;color:var(--ef-text-faint);margin-top:8px;line-height:1.5">
            {subtitle}
          </div>
        )}
      </div>
