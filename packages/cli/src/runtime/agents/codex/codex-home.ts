...
import { type LocalMcpConfig, loadLocalMcpConfig } from './local-mcp.js';
...
export interface CodexHomeOptions {
  ...
  /** Durable member-local stdio MCP config. Defaults under XDG_CONFIG_HOME. */
...
export function setupCodexHome(options: CodexHomeOptions): CodexHomeHandle {
  ...
  // Validate durable input before creating the ephemeral home: a bad
  // member config must fail with no cleanup owed.
  const localMcp = loadLocalMcpConfig({
    path: options.localMcpConfigPath,
  ...
  writeFileSync(configPath, renderConfigToml(options, localMcp), { mode: 0o600 });
 ...
 *     other way
 */
function renderConfigToml(opts: CodexHomeOptions, localMcp: LocalMcpConfig): string {
  ...
  const lines = [
    ...
    `# Local stdio MCP source: ${localMcp.path}`,
    '',
    ...renderLocalMcpServers(localMcp),
    '[mcp_servers.csuite]',
...
function renderLocalMcpServers(config: LocalMcpConfig): string[] {
  const lines: string[] = [];
  for (const [name, server] of Object.entries(config.servers)) {
    ...
    if (server.startupTimeoutSec !== undefined) {
      lines.push(`startup_timeout_sec = ${String(server.startupTimeoutSec)}`);
    }
    if (server.toolTimeoutSec !== undefined) {
      lines.push(`tool_timeout_sec = ${String(server.toolTimeoutSec)}`);
    }
