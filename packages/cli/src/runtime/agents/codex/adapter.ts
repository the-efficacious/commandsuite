...
export async function spawnCodex(opts: CodexSpawnOptions): Promise<CodexSpawnResult> {
  ...
  // The briefing is more than its `instructions` string: process rules
  // ride in their own field, because inside `instructions` they would
  // exceed the client-side 8192 cap and the runner would not start.
  const fixedContext = composeFixedContext(opts.briefing);
  const developerInstructions = fixedContext.length > 0 ? fixedContext : undefined;
