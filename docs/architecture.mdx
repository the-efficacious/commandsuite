...
registers in-process Agent SDK hook callbacks that forward
Pre/PostToolUse payloads to the CaptureHost's loopback hook server;
those carry the tool **result** content OTEL omits, and also drive
the busy signal:
