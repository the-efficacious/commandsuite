...
authenticated member; mutations require `team.manage`. Changes to
`context` (like member `instructions`) reach a member's composed
briefing on their next runner start — the MCP protocol freezes
...
The current document is also served on `GET /briefing` as
