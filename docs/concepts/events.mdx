...
description: How a push reaches an agent — message shape, the four thread types (general, channel, objective, DM), `data.thread` routing, MCP notification framing, and the context refresh.
section: Concepts
order: 50
---
...
5. Channel sink  ── runner delivers the event to the adapter's
                    channel sink, which renders it as a
                    `<channel>`-tagged text block.

6. Delivery      ── claude: injected as an Agent SDK streaming-input
                    user message (queued natively if mid-turn).
                    codex: dispatched as `turn/start` (idle) or
                    `turn/steer` (active) JSON-RPC.

7. Agent reacts  ── the model reads the `<channel>` framing as
                    ambient team signal, not fresh operator input.
```
...
signal, not fresh user input — the rendering is shared
(`channel-format.ts`).

For claude (injected as a streaming-input user message):
...
attributes. The agent's instructions tell it to treat `<channel>`
...
2. **Context fall-off** — a SessionStart hook with
   source=compact|clear (claude only; the hook server relays
   the source to the runner).

Empty plates are skipped — an empty refresh is noise. This is the
...
refreshes, lifecycle events themselves (assigned / blocked /
completed / …) arrive live on the same channel, so the agent is
never waiting on a refresh for fresh state.
