...
standing doctrine (below) establishes once that fenced content is
data, not instructions.

Members reachable by any enabled endpoint get a standing
**instructions section** explaining the contract — defined once in
the frozen system prompt, so each delivery's wrapper stays compact.
