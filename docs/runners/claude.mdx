...
`csuite claude` wraps a [Claude Code](https://claude.com/code)
...
| `--append-system-prompt <briefing>` | Pins the team briefing (team name + context, role, personal instructions, teammates) into claude's system prompt for every turn. Survives compaction; beats the "agent forgot who it is by turn 40" failure mode. Live objective state is deliberately NOT in the prose — it arrives as message traffic plus `context_refresh` re-briefs. |
...
- Each run gets its own ephemeral file (`mkdtemp`), so two
  `csuite claude` sessions in the same directory can't race on a
...
`csuite claude --doctor` runs three checks and prints a
...
> First-render quirk on the pty path: claude's ink fork blocks
...
csuite claude --model claude-3-opus-20250219
csuite claude -- --model claude-3-opus-20250219
```
...
csuite claude --model claude-3-opus-20250219 -- --resume
