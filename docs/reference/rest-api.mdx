...
of every teammate, and the caller's currently-open objectives. The
response also names the instruction blocks it composed — `blocks`
carries a per-block sha256 descriptor for each of team_context,
role_description, personal_instructions, and process_document, and
`composedSha256` is the canonical hash of the composed content that
the broker tracks restart-pending against.
...
`context_refresh` push, and `permissions` decides which
...
|---|---|
| **Auth** | dual |
| **Returns** | `{ teammates: Teammate[], connected: Presence[], restartPending?: string[] }` |

Public projection of teammates plus a parallel list of live
presence (connection count, last-seen, busy flag). `restartPending`
lists members whose current composed instructions differ from what
their live session was issued — their runner restarts the agent at
its next idle boundary to pick the edit up. For the presence model
see [presence](/docs/concepts/presence).
...
changed). Each affected runner restarts its agent at the next idle
boundary under the new text; `GET /roster` lists those members in
