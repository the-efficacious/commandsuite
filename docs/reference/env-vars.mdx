...
| `CLAUDE_PATH` | Agent SDK's bundled CLI | Override the Claude Code executable the Agent SDK drives. Used by `csuite claude` and `--doctor`; unset means the CLI shipped in `@anthropic-ai/claude-agent-sdk-<platform>-<arch>`. |
...
`csuite claude` and `csuite codex` set these on the spawned
...
### claude — OpenTelemetry export (runner-managed)
...
come from OTEL — they come from Claude Code hooks. The runner
registers in-process Agent SDK hook callbacks that forward PreToolUse
/ PostToolUse payloads to a loopback hook server the capture host
owns; that server emits the `tool_action` result content and drives
the busy signal. The hook endpoint is an in-process loopback URL, not
an env var.
...
- `ANTHROPIC_API_KEY` — claude reads this directly. The
