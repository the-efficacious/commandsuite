...
| `CLAUDE_PATH` | `which claude` | Path to the Claude Code binary. Used by `csuite claude` and `--doctor`. |
| `CODEX_PATH` | `which codex` | Path to the OpenAI Codex binary. Used by `csuite codex`. |
| `CSUITE_CLAUDE_MCP_MODE` | `flag` | How `csuite claude` wires the csuite MCP server into Claude Code. `flag` passes it via `--mcp-config` (ephemeral); `inject` writes it into the Claude MCP config instead. |
...
| `TERM` | Forwarded to the pty when claude uses node-pty (default `xterm-256color`). |
